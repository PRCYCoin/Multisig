...
const char* BIP71_MIMETYPE_PAYMENT = "application/prcycoin-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/prcycoin-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/prcycoin-paymentrequest";
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in prcycoin: URIs
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // prcycoin: URI
    {
        ...
        if (GUIUtil::parseBitcoinURI(s, &recipient)) {
            ...
            emit message(tr("URI handling"),
                tr("URI cannot be parsed! This can be caused by an invalid PRCY address or malformed URI parameters."),
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach (const PAIRTYPE(CScript, CAmount) & sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom prcycoin addresses are not supported
